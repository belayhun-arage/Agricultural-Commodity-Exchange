...
class RegisterMerchantForm extends StatefulWidget {
  static String RouteName = "merchant/new";
  const RegisterMerchantForm({Key? key}) : super(key: key);

  @override
  State<RegisterMerchantForm> createState() => _RegisterMerchantFormState();
}

class _RegisterMerchantFormState extends State<RegisterMerchantForm> {
                ...
                child: InkWell(
                  onTap: () async {
                    ...
                    if (validated) {
                      ...
                      if (state is NewMerchantPostedState) {
                        UserBloc user = BlocProvider.of<UserBloc>(context);
                        final Admin theAdmin =
                            (user.state as Authenticated).user as Admin;
                        context
                            .read<AdminsBloc>()
                            .add(GetAllMerchantsEvent(admin: theAdmin));
                        Navigator.pop(context);
                      }
                      if (state is FailedToCreateNewMerchantState) {
                        // context.read<AdminsBloc>().add(FailedToCreateNewMerchantState());
                        Navigator.pop(context);
                      }
                    }
                  },
